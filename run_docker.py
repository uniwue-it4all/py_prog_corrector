...
if correction_type == 'simplified':
    ...
    if not user_solution_file_path.exists() or not test_main_file_path.exists() or not test_data_file_path.exists():
        print(f'not all files were provided!')
        exit(10)

    mount_points = [
        MountPoint(result_file_path, mount_base_path / 'result.json'),
        MountPoint(user_solution_file_path, mount_base_path / user_sol_file_name, read_only=True),
        MountPoint(test_main_file_path, mount_base_path / 'test_main.py', read_only=True),
    ...
    if not solution_file_path.exists() or not test_file_path.exists() or not test_data_file_path.exists():
