...
complete_test_config: CompleteTestConfig = CompleteTestConfig.parse_from_json(
    loaded_json
)
...
for test_config in complete_test_config.test_configs:
    file_result, result = run_test(
        ...
        test_file_content,
        folder_name,
        complete_test_config.file_name,
        test_file_name,
    )
...
result_file_path.write_text(json_dumps(complete_result.to_json_dict(), indent=2))
